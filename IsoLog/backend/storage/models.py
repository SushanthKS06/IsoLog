"""
IsoLog Database Models

SQLAlchemy models for events, alerts, and blockchain hashes.
"""
...
class Event(Base):
    """
    Normalized log event following ECS schema.
    
    Stores structured log data after parsing and normalization.
    """
...
class Alert(Base):
    """
    Detection alert generated by the detection engine.
    
    Links to the source event and contains detection metadata.
    """
    ...
    # MITRE ATT&CK mapping (JSON arrays)
...
class BatchHash(Base):
    """
    Blockchain hash record for log batch integrity.
    
    Stores Merkle root hashes of event batches for tamper detection.
    """
    ...
    # Blockchain transaction (if synced to external chain)
    ...
    def to_dict(self) -> dict:
        """Convert batch hash to dictionary."""
        return {
            "id": self.id,
...
class SystemState(Base):
    """
    System state tracking for ML models, updates, etc.
    """
